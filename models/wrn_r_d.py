...
class ArchConditionalBatchNorm(torch.nn.Module):
    ...
    def forward(self, input, arch_ids):
        ...
        if self.affine:
            ...
            out = torch.addcmul(arch_ids.matmul(self.bias).view(shape), arch_ids.matmul(self.weight).view(shape), out)
        return out
...
class Stage(nn.Module):
  ...
  def forward(self, x, arch_ids):
    if self.learn_aggr:
        mask = arch_ids.matmul((self.adj * self.sigmoid(self.mean_weight)).view(self.adj.shape[0], -1)).view(arch_ids.shape[0], self.adj.shape[1], self.adj.shape[2])
    else:
        mask = arch_ids.matmul(self.adj.view(self.adj.shape[0], -1)).view(arch_ids.shape[0], self.adj.shape[1], self.adj.shape[2])
...
class ArchPredictor(nn.Module):
  ...
  def __init__(self, C, arch_nums, gradient_est='st'):
    super(ArchPredictor, self).__init__()
    self.gradient_est = gradient_est
    self.fn = nn.Sequential(
      nn.BatchNorm2d(C),
      nn.ReLU(inplace=True),
      nn.Conv2d(C, 512, 3, stride=1, padding=1, bias=False), ###here
      nn.BatchNorm2d(512),
      nn.ReLU(inplace=True),
      ...
      nn.ReLU(inplace=True), 
      nn.Linear(64, arch_nums)
    )

  def forward(self, x):
    x = self.fn(x)
    if self.gradient_est == 'st':
        x_soft = x.softmax(-1)
        x_sample = torch.multinomial(x_soft, 1)
        x_hard = torch.zeros_like(x_soft)
        x_hard.scatter_(1, x_sample, 1)
        y = (x_hard - x_soft).detach() + x_soft
    elif self.gradient_est == 'gsm':
        gumbels = -torch.empty_like(x).exponential_().log()  # ~Gumbel(0,1)
        gumbels = (x + gumbels) / 1.
        y = gumbels.softmax(-1)
    elif self.gradient_est == 'sm':
        y = x.softmax(-1)
    else:
        raise Error
    return x, y
...
class WRN(nn.Module):
    def __init__(self, args, num_classes=10):
        ...
        self.arch_predictor = ArchPredictor(n_channels[0], self.arch_nums, args.gradient_est)
    ...
    def forward(self, x, return_archs=False):
        
        x = self.conv_3x3(x)

        arch_logits, arch_ids = self.arch_predictor(x)
        
        x = self.norm(x, arch_ids)
        ...
        if self.training and self.aux:
            ...
            if return_archs:
                return self.classifier(x), logits_aux, arch_ids
            else:
                return self.classifier(x), logits_aux
        else:
            x = self.stage_3(x, arch_ids)
            if return_archs:
                return self.classifier(x), arch_ids
            else:
